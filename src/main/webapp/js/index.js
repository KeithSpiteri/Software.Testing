...
$(function() {
	...
	$('#user').blur(function(event) {
	    ...
	    if(message){input.tipsy({opacity:0});input.removeClass("invalid").addClass("valid");}
	...
	$('#pass').blur(function(event) {
	    var input=$(this);
	    var message=$(this).val();
	    if(message.length > 7){input.tipsy({opacity:0});input.removeClass("invalid").addClass("valid");}
	    else{input.removeClass("valid").addClass("invalid");}  
	});
	...
	$('#name').blur(function(event) {
	    ...
	    if(valid){input.tipsy({opacity:0});input.removeClass("invalid").addClass("valid");}
	    else{input.removeClass("valid").addClass("invalid");}  
	});
	...
	$('#CCN').blur(function(event){
        var input=$(this);
        var ccn = input.val();        
        if(ccn && luhnValidation(ccn)){input.tipsy({opacity:0}); input.removeClass("invalid").addClass("valid");}
        else{input.addClass("invalid").removeClass("valid");}
	});
	
	$("#CCN").keydown(function (e) { //Don't allow any inputs which are not numbers
        // Allow: backspace delete tab escape enter and .
        if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) != -1 ||
            // Allow: Control + Characters
            ((e.keyCode == 65 || e.keyCode == 97 
            		|| e.keyCode == 67 || e.keyCode == 99
            		|| e.keyCode == 86 || e.keyCode == 166
            		|| e.keyCode >= 35 && e.keyCode <= 39)) && e.ctrlKey == true) {
                 return;
        	}
	...
	$("#CVV").keydown(function (e) { //Don't allow any inputs which are not numbers
        // Allow: backspace delete tab escape enter and .
        if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) != -1 ||
            // Allow: Control + Characters
            ((e.keyCode == 65 || e.keyCode == 97 
	...
	function luhnValidation(ccn) {
		  ...
		  while (length > 0) { 
			  length--;
			  string += "01234567890246813579".charAt(ccn.charAt(length)*1 + ((bool=!bool)?0:10)) * 1; 
		  }
		  return (string % 10 == 0);
	}
	
	 $('#expiry').blur(function(event){
		 var input=$(this);
	     var date = input.val();
	     var actual = new Date(date);
	     var curr_date = new Date();
	     if(determineAge(actual) < 0 || (determineAge(actual) == 0 && (actual.getMonth() - curr_date.getMonth()) >= 0)){
	      	input.tipsy({opacity:0}); 
	       	input.removeClass("invalid").addClass("valid");
	     }
	     else{input.addClass("invalid").removeClass("valid");}
	...
	$("#register_submit").click(function(event){
	    ...
	    for (var input in form_data){
	        ...
	        if (!valid){
	        	...
	        	element.tipsy({fallback: message ,opacity:0.8, gravity: 'w'});
	        	error_free=false;
	        	error_element.tipsy();
